...
#    parser_easyphylo.add_argument('-m', '--mafft', required=False, default ="--auto", help='MAFFT: Mafft alignment options. Default  --auto')
    ...
    parser_blastology.add_argument('--cluster_method', required=False, default = 'mmseqs2', help='Clustering of the joint hit set: mmseqs2 (connected-component clustering, --cluster-mode 1 -- needs neither a neighbour cap nor an MCL inflation, so there is no per-family threshold to justify) or diamond_mcl (all-vs-all graph + MCL; requires --per_species_n and inflation <= 1.2). Default [mmseqs2] since 2026-08-20; was diamond_mcl.')

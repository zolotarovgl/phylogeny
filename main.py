...
def do_hmmsearch(hmm, out, fasta, cpu, threshold):
...
def merge_results(prefix,gene_family_name, searches_dir, fasta_file):
    ...
    if num_hit_genes == 0:
        ...
        cmd = (
            ...
            f"&& rm -f {searches_dir}/{prefix}.{fam_id}.domains.csv.tmp"
        )
        ...
        cmd = (
            ...
            f"&& rm {searches_dir}/{prefix}.{fam_id}.domains.csv.tmp2"
        )
        cmd = cmd.replace("@",'"')
        print(cmd)
        subprocess.run(cmd, shell=True, executable='/bin/bash', check=True)
...
def search(fasta_file, gene_family_info, gene_family_name, config):
...
if __name__ == "__main__":
    ...
    parser_search = subparsers.add_parser('search', help='Run search')
    ...
    if args.command == 'search':
        ...
        search(args.fasta, args.gene_family_info, args.gene_family_name, config)
